    ...
    for file_item in uploaded_files:
        ...
        if ext == ".zip":
            try:
                ...
                str_lit.error(f"Error reading ZIP file '{filename}': {e}")
...
def run_one_image(filename, image_bytes, user_api_key, workspace_name, workflow_id, run_id, run_started_at):
    """Executes workflow using InferenceHTTPClient dynamically matching the user's account configuration."""
...
if "run_complete" not in str_lit.session_state:
    str_lit.session_state.run_complete = False
if "df_results" not in str_lit.session_state:
    str_lit.session_state.df_results = None
if "df_summary" not in str_lit.session_state:
    str_lit.session_state.df_summary = None
if "excel_results_bytes" not in str_lit.session_state:
    str_lit.session_state.excel_results_bytes = None
if "excel_summary_bytes" not in str_lit.session_state:
    str_lit.session_state.excel_summary_bytes = None
...
str_lit.title("🛥️ Boat Registration & Classification System")
str_lit.markdown("Upload individual images, dropped folders, or ZIP archives for automated batch processing.")

# Sidebar Configuration for On-the-Fly Account Entry
with str_lit.sidebar:
    str_lit.header("⚙️ Configuration")
    
    api_key = str_lit.text_input(
        "Roboflow API Key", 
        type="password",
        help="Enter your personal Roboflow Private API Key."
    )
    workspace_name = str_lit.text_input(
        "Workspace Name",
        value="blakes-workspace-efak8",
        help="Your Roboflow Workspace URL slug."
    )
    workflow_id = str_lit.text_input(
        "Workflow ID",
        value="boat-registration-and-classification-combination",
        help="The specific Workflow ID you want to run."
    )
    
    str_lit.divider()
    max_workers = str_lit.slider("Parallel Processing Threads", min_value=1, max_value=10, value=3)

# File / Folder Dropzone
str_lit.subheader("📁 Input Files")
uploaded_items = str_lit.file_uploader(
    "Drag and drop image files, folders, or ZIP archives here:",
    type=["jpg", "jpeg", "png", "bmp", "tif", "tiff", "webp", "zip"],
    accept_multiple_files=True
)

server_output_dir = str_lit.text_input("Optional Server Save Path (Leave blank to download via browser):")
...
if str_lit.button("🚀 Process Batch", type="primary", disabled=not (uploaded_items and api_key and workspace_name and workflow_id)):
    str_lit.session_state.run_complete = False
    ...
    with str_lit.status("Initializing processing session...", expanded=True) as status:
        
        status.update(label="Extracting image batch...", state="running")
        image_batch = extract_images_from_uploads(uploaded_items)
        str_lit.write(f"Total images identified for processing: **{len(image_batch)}**")

        if not image_batch:
            str_lit.error("No valid image files found in the upload.")
            str_lit.stop()
    ...
    if server_output_dir:
        ...
        str_lit.success(f"Both Excel files saved to server at: `{server_output_dir}`")

    str_lit.session_state.df_results = df_res
    str_lit.session_state.df_summary = df_sum
    str_lit.session_state.excel_results_bytes = excel_res_bytes
    str_lit.session_state.excel_summary_bytes = excel_sum_bytes
    str_lit.session_state.run_complete = True
...
if str_lit.session_state.run_complete:
    str_lit.divider()
    str_lit.header("📊 Results & Generated Excel Files")

    tab1, tab2 = str_lit.tabs(["📄 Detailed Clean Results", "📅 Registration Daily Summary"])

    with tab1:
        str_lit.dataframe(str_lit.session_state.df_results, use_container_width=True)

    with tab2:
        str_lit.dataframe(str_lit.session_state.df_summary, use_container_width=True)

    str_lit.subheader("📥 Download Output Excel Files")
    col1, col2 = str_lit.columns(2)

    with col1:
        str_lit.download_button(
            label="📘 Download Clean Results (.xlsx)",
            data=str_lit.session_state.excel_results_bytes,
    ...
    with col2:
        str_lit.download_button(
            ...
            data=str_lit.session_state.excel_summary_bytes,
