...
DETAIL_COLUMNS = [
    ...
    "best_visible_registration_candidate",
    "boat_count",
    "boat_detected",
    "date_and_time",
    "output_line",
...
def run_one_image(
    ...
    so their Roboflow account credits are used.
    """
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            row = {
                ...
                "best_visible_registration_candidate": clean_scalar(output.get("best_visible_registration_candidate")),
...
def derive_summary_registration(row):
    """
    Determines the registration value used in the daily summary.

    Priority:
    1. Confirmed registration number
    2. Workflow's best visible registration candidate
    3. Partial registration candidate fallback
    4. UNDETECTED
    """
    confirmed_registration = str(row.get("registration_number") or "").strip()
    best_visible_candidate = str(row.get("best_visible_registration_candidate") or "").strip()
    partial_candidate = str(row.get("partial_registration_candidate") or "").strip()

    if confirmed_registration:
        return confirmed_registration

    if best_visible_candidate:
        return best_visible_candidate
...
def derive_summary_registration_source(row):
    """
    Determines the exact source label used in the daily summary.

    Desired output values:
    - confirmed_registration_number
    - best_visible_registration_candidate
    - undetected
    """
    confirmed_registration = str(row.get("registration_number") or "").strip()
    ...
    if confirmed_registration:
        return "confirmed_registration_number"

    if best_visible_candidate or partial_candidate:
        return "best_visible_registration_candidate"

    return "undetected"


def is_confirmed_row(row):
    """Counts rows where a confirmed registration number was found."""
    confirmed_registration = str(row.get("registration_number") or "").strip()
    return bool(confirmed_registration)


def is_partial_or_review_row(row):
    """
    Counts rows where no confirmed registration was found,
    but the workflow found a best visible registration candidate.
    """
    confirmed_registration = str(row.get("registration_number") or "").strip()
    best_visible_candidate = str(row.get("best_visible_registration_candidate") or "").strip()
    partial_candidate = str(row.get("partial_registration_candidate") or "").strip()

    return bool(not confirmed_registration and (best_visible_candidate or partial_candidate))


def is_undetected_row(row):
    """Counts rows where no confirmed registration or visible candidate was found."""
    confirmed_registration = str(row.get("registration_number") or "").strip()
    best_visible_candidate = str(row.get("best_visible_registration_candidate") or "").strip()
    partial_candidate = str(row.get("partial_registration_candidate") or "").strip()

    return not confirmed_registration and not best_visible_candidate and not partial_candidate
...
def build_daily_summary(df_results):
    ...
    df_work["summary_registration"] = df_work.apply(derive_summary_registration, axis=1)
    ...
    for (capture_date, summary_registration), group in grouped:
        ...
        summary_registration_source = source_values[0] if source_values else "undetected"
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(future_to_file):
                ...
                st.write(
                    ...
                    f"best_visible={res.get('best_visible_registration_candidate')}"
                )
