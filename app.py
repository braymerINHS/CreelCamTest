...
def ensure_workflow_exists(api_key, target_workspace, source_workspace, workflow_id):
    """
    Checks if the workflow exists in the user's workspace.
    If missing, attempts to clone/fork the workflow definition into their workspace.
    """
    headers = {"Content-Type": "application/json"}
    
    # 1. Check if user already has access to the workflow
    check_url = f"https://api.roboflow.com/{target_workspace}/workflows/{workflow_id}?api_key={api_key}"
    res = requests.get(check_url, headers=headers)
    if res.status_code == 200:
        return True, f"Found existing workflow `{workflow_id}` in workspace `{target_workspace}`."

    # 2. Fetch definition from source workspace
    ...
    if source_res.status_code != 200:
        return True, f"Using source workflow endpoint: `{source_workspace}/{workflow_id}`."
    ...
    if create_res.status_code in [200, 201]:
        return True, f"Successfully cloned `{workflow_id}` into workspace `{target_workspace}`!"
    else:
        return True, f"Running inference directly against `{source_workspace}`."


def extract_images_from_uploads(uploaded_files):
    """
    Extracts raw image bytes while preserving folder hierarchy and relative file paths.
    """
                    ...
                    for zip_info in z.infolist():
                        if not zip_info.is_dir():
                            ...
                            if zip_ext in valid_extensions:
                                ...
                                # Preserve relative path in filename column
...
st.markdown("Upload individual images, folders, or ZIP archives for automated high-detail analysis.")
...
with st.sidebar:
    ...
    target_workspace = st.text_input("Your Roboflow Workspace ID", value=SOURCE_WORKSPACE)
...
if st.button("🚀 Process Batch", type="primary", disabled=not (uploaded_items and api_key)):
    ...
    with st.status("Initializing workflow & preparing full-fidelity images...", expanded=True) as status:
        ...
        # Step A: Check / Fork Workflow
        st.write("Verifying Roboflow workflow access...")
        success, wf_msg = ensure_workflow_exists(api_key, target_workspace, SOURCE_WORKSPACE, workflow_id)
        st.write(wf_msg)
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_file = {
                executor.submit(run_one_image, fn, bytes_data, api_key, target_workspace, workflow_id): fn
