...
def build_daily_summary(df_results):
    ...
    status_col = None
    for candidate in ["detection_status", "review_status", "status_type", "registration_status"]:
        if candidate in df_work.columns:
    ...
    for (cap_date, reg_val), group in groups:
        ...
        if status_col and status_col in sorted_group.columns:
            ...
            review_cnt = s_series.str.contains("REVIEW|PARTIAL", regex=True).sum()
            undetected_cnt = s_series.str.contains("UNDETECTED|NONE", regex=True).sum()
        else:
            if str(reg_val).upper() in ["UNDETECTED", "NONE", "UNKNOWN", ""]:
                confirmed_cnt = 0
                review_cnt = 0
        ...
        source_val = sorted_group["summary_registration_source"].iloc[0] if "summary_registration_source" in sorted_group.columns else "AUTOMATED_OCR"
