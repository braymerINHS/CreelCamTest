...
def run_one_image(filename, image_bytes, workflow_spec, user_api_key):
    """
    Executes the workflow INLINE on Roboflow's inference servers.
    The spec is passed in the payload, billing usage directly to user_api_key.
    """
    b64_image = base64.b64encode(image_bytes).decode("utf-8")
    
    # Generic Roboflow workflow execution endpoint
...
if st.button("🚀 Process Batch", type="primary", disabled=not (uploaded_items and api_key)):
    ...
    with st.status("Initializing processing session...", expanded=True) as status:
        ...
        # Step A: Load local workflow specification
        ...
        status.update(label="Processing images via Roboflow API (billed to your key)...", state="running")
