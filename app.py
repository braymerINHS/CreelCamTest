...
def ensure_workflow_exists(api_key):
    ...
    3. If missing, clones template and renames it to 'boat-registration-and-classification-combination-test'.
    """
    headers = {"Content-Type": "application/json"}
    target_workspace = get_user_workspace(api_key)
    
    # If we successfully detected the user's workspace
    if target_workspace and target_workspace != SOURCE_WORKSPACE:
        # Check if the test workflow already exists in user's workspace
        ...
        # Fetch template definition from source workspace
        source_url = f"https://api.roboflow.com/{SOURCE_WORKSPACE}/workflows/{SOURCE_WORKFLOW_ID}?api_key={api_key}"
        source_res = requests.get(source_url, headers=headers)
        
        if source_res.status_code == 200:
            workflow_data = source_res.json()
            
            # Override ID and name to the requested test name
            if isinstance(workflow_data, dict):
                workflow_data["id"] = TARGET_WORKFLOW_ID
                workflow_data["name"] = TARGET_WORKFLOW_ID
            
            # Fork workflow into target workspace with new test ID
            ...
            if create_res.status_code in [200, 201]:
                return target_workspace, TARGET_WORKFLOW_ID, f"Successfully created `{TARGET_WORKFLOW_ID}` in workspace `{target_workspace}`!"

    # Fallback to source workspace if auto-detection or cloning is restricted
    return SOURCE_WORKSPACE, SOURCE_WORKFLOW_ID, f"Running inference directly against template workflow (`{SOURCE_WORKSPACE}/{SOURCE_WORKFLOW_ID}`)."
...
# Minimalist Sidebar with Interactive Help Tooltip
with st.sidebar:
...
if st.button("🚀 Process Batch", type="primary", disabled=not (uploaded_items and api_key)):
    ...
    with st.status("Initializing workflow & preparing full-fidelity images...", expanded=True) as status:
        ...
        # Step A: Auto-detect Workspace & Clone test workflow if needed
