...
def ensure_workflow_exists(api_key):
    ...
    # Step 2: If missing, load local JSON spec from repo files and deploy it
    ...
    # Force the workflow ID and name to match the target test name inside the payload
    if isinstance(workflow_spec, dict):
        workflow_spec["id"] = TARGET_WORKFLOW_ID
        workflow_spec["name"] = TARGET_WORKFLOW_ID

    # Fixed URL: Appended TARGET_WORKFLOW_ID to the POST endpoint route
    create_url = f"https://api.roboflow.com/{target_workspace}/workflows/{TARGET_WORKFLOW_ID}?api_key={api_key}"
