...
# PAGE SETUP & SIDEBAR
# =========================
st.set_page_config(page_title="Boat Registration & Classification", layout="wide")
st.title("🚤 Boat Registration & Classification Tool")
st.write("Drag and drop your dataset below to run automated registration OCR and boat classification.")

st.sidebar.header("🔑 Roboflow Credentials")
st.sidebar.info("Inference credits are billed directly to your Roboflow account.")

user_api_key = st.sidebar.text_input(
    "Roboflow Private API Key",
    type="password",
    help="Find your key at app.roboflow.com/settings/api"
)

workspace_name = st.sidebar.text_input("Workspace Name", value="blakes-workspace-efak8")
workflow_id = st.sidebar.text_input("Workflow ID", value="boat-registration-and-classification-combination")
...
use_cache = st.sidebar.checkbox("Use Workflow Cache", value=False)
...
def run_one_image(uploaded_file, client, workspace, workflow, run_id, run_started_at):
    last_error = ""
...
if uploaded_files:
    ...
    if st.button("Start Batch Analysis", type="primary"):
        if not user_api_key:
            st.error("Please enter your Roboflow Private API Key in the sidebar.")
            st.stop()
        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            future_to_file = {
                executor.submit(
                    run_one_image, file, client, workspace_name, workflow_id, run_id, run_started_at
                ): file
