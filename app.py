...
# ==========================================
# Extracted from your Roboflow share payload
SOURCE_WORKSPACE_ID = "eKLbGNlr6ZWrmRK2xbVNvXJ9Agl1"
SOURCE_WORKFLOW_ID = "OIAz4HqFzQaDQ9XLob2Q"

# Target named endpoint for the user's workspace
...
def ensure_user_has_forked_workflow(user_workspace, api_key):
    ...
    2. If missing, auto-forks the master workflow using Roboflow's fork API.
    """
    headers = {"Content-Type": "application/json"}
    
    # Check if workflow already exists in user's workspace
    ...
    # If missing, trigger programmatic fork from master workflow
    ...
    if fork_res.status_code in [200, 201] or "already exists" in fork_res.text.lower():
        return True, f"Successfully forked workflow into workspace `{user_workspace}`!"
    else:
...
def run_one_image(filename, image_bytes, api_key, workspace):
    """
    Sends original full-detail image bytes to the user's local workflow endpoint.
    """
