...
# ==========================================
# Every visitor will have the workflow saved under this identical ID in their account
...
def load_workflow_spec():
    """Loads the workflow JSON specification from the local app directory."""
...
def get_visitor_workspace_id(user_api_key):
    """
    Retrieves the visitor's Roboflow workspace handle using their API Key.
    """
...
def fork_workflow_to_visitor_workspace(visitor_workspace, user_api_key, workflow_spec):
    """
    Programmatically creates ('forks') or updates the workflow inside the visitor's 
    workspace using their API key.
    """
    deploy_url = f"https://api.roboflow.com/{visitor_workspace}/workflows/{WORKFLOW_ID}?api_key={user_api_key}"
    
    # PUT creates the workflow if missing, or overwrites it if it already exists
    response = requests.put(deploy_url, json=workflow_spec, timeout=20)
    
    # If key lacks permission or payload is rejected, raise an explicit error
...
def run_one_image(filename, image_bytes, visitor_workspace, user_api_key):
    """
    Executes the workflow endpoint now residing directly inside the VISITOR'S workspace.
    """
    b64_image = base64.b64encode(image_bytes).decode("utf-8")
    
    # Correct Roboflow Hosted Workflow Inference URL
    url = f"https://detect.roboflow.com/infer/workflows/{visitor_workspace}/{WORKFLOW_ID}?api_key={user_api_key}"
...
if st.button("🚀 Process Batch", type="primary", disabled=not (uploaded_items and api_key)):
    ...
    with st.status("Initializing workflow deployment and processing session...", expanded=True) as status:
        
        # Step A: Load workflow specification
        status.update(label="Reading workflow template from application...", state="running")
        try:
        ...
        # Step B: Discover visitor workspace handle
        ...
        # Step C: Fork / Create workflow inside visitor workspace
        status.update(label=f"Deploying workflow into '{visitor_workspace}'...", state="running")
        try:
            fork_workflow_to_visitor_workspace(visitor_workspace, api_key.strip(), workflow_spec)
            st.write(f"Workflow **`{WORKFLOW_ID}`** successfully created/synced in **`{visitor_workspace}`**.")
        except Exception as e:
            st.error(f"Failed to fork workflow into workspace: {e}")
            st.stop()

        # Step D: Extract uploaded image batch
        ...
        # Step E: Execute batch against their newly created workflow
        status.update(label="Processing images (billed to your workspace)...", state="running")
